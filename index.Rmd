...
`r png::writePNG(pdf_render_page("./raw_data/us_regdiv.pdf", page = 1, dpi = 300),"./figures/census_regions.png")`
...
Using data from the [Behavioral Risk Factor Surveillance System](https://www.cdc.gov/brfss/index.html), data about health factors and living condition of the non-White population were analyzed. Figure 11 shows the significant negative correlation between pneumonia vaccination and per capita COVID deaths among the elderly non-White population. The correlations were similarly negatively-correlated for the elderly White population, but orders of magnitude weaker (data not shown).
...
```{r graphVax, include = TRUE, fig.cap = "**Figure: 11:** "}
...
```{r graphRural, include = TRUE, fig.cap = "**Figure:**"}
data <- read_csv("./output/state_health.csv") %>%
  filter(!isWhite) %>%
  inner_join(state.regions, by = "region")
...
```{r graphPopDensity, include = TRUE, fig.cap = "**Figure:**"}
...
As shown in figures x-x, several measures of density are significantly correlated with the severity of COVID pandemic. Statewide measures of the percentage of the population living in metropolitan areas, including (figure x) or excluding (figure x) the suburbs, showed strong positive relationships with per capita deaths from COVID among non-White populations of all ages. The expected opposite trend holds for the statewide percentage of the population living in rural areas (figure x). More directly, population density (figure x) showed the strongest correlation with per capita COVID deaths across all age groups, races, and variables investigated (data not shown). These data are in line with the expectation that human-to-human diseases kill most readily in areas where humans are closer together.
