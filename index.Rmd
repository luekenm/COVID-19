...
if(!file.exists("./raw_data/States-Votes.xlsx")) {
  download.file(url = "https://www.fec.gov/documents/1890/federalelections2016.xlsx", destfile = "./raw_data/States-Votes.xlsx", mode = "wb", quiet = TRUE) 
}
...
read_excel("./raw_data/States-Votes.xlsx", sheet = "Table 2. Electoral &  Pop Vote") %>%
  ...
  select(State, Margin) %>%
  write_csv("./output/States_info.csv")
```
...
# Close enough to correct estimates of state pop using lazy code copying
statePop <- read_csv("./output/census_state_age_race_data.csv") %>%
  group_by(region) %>%
  summarize(pop = sum(Pop_White + `Pop_Non-White`))
