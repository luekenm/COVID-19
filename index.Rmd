...
if(!file.exists("./raw_data/states_governors.csv")) {
  wget(url = "https://github.com/luekenm/COVID-19/raw_data/states_governors.csv", destfile = "./raw_data/states_governors.csv")
}
...
if(!file.exists("./raw_data/states_whole_foods.csv")) {
  wget(url = "https://github.com/luekenm/COVID-19/raw_data/states_whole_foods.csv", destfile = "./raw_data/states_whole_foods.csv")
}
...
if(!file.exists("./raw_data/us_regdiv.pdf")) {
  wget(url = "https://www2.census.gov/geo/pdfs/maps-data/maps/reference/us_regdiv.pdf", destfile = "./raw_data/us_regdiv.pdf", )
}
...
pop <- read_csv("./output/cdc_state_covid_race_data.csv") %>%
  ...
  StateChoropleth$new()
pop$title = "Percentage of Population that is non-White"
pop$ggplot_scale = scale_fill_brewer(type = "seq", palette = 2, name = "Percent\nPopulation", drop = FALSE, na.value = "black")
...
prop <- read_csv("./output/cdc_state_covid_race_data.csv") %>%
  ...
  StateChoropleth$new()
prop$title = "Percent of COVID Deaths from non-White Population"
prop$ggplot_scale = scale_fill_brewer(type = "seq", palette = 8, name = "Percent\nDeaths", drop = FALSE, na.value = "black")
...
ggplot(filter(mapData, region == "michigan"), aes(x = long,
  ...
  labs(title = "Ratio of Proportion of COVID Deaths to Proportion of Population",
       ...
       y = "") +
  coord_map() +
  ggsave("./figures/ratio_county_mi.png", width = 6, height = 6)
```
...
ggplot(filter(mapData, region == "new york"), aes(x = long,
  ...
  labs(title = "Ratio of Proportion of COVID Deaths to Proportion of Population",
       ...
       y = "") +
  coord_map() +
  ggsave("./figures/ratio_county_ny.png", width = 7, height = 6)
```
...
![Census Regions](./figures/census_regions.png)
...
read_csv("./output/cdc_state_covid_age_race_data.csv") %>%
  ...
  theme_linedraw() +
  ylim(0, 4) +
  ggsave("./figures/rdi_regions.png")
```
...
```{r plotCOVIDStates, include = TRUE, fig.cap = "**Figure 8:** RDI for each Census Region by Age and State"}
...
**Table 1:** Age-Based RDI and Political Data by State
...
```{r plotCOVIDGovernorSex, include = TRUE, fig.cap = "**Figure 9:** RDI of States Plotted by Age and Sex of Governor"}
# By Governor Sex
read_csv("./raw_data/states_governors.csv") %>%
  left_join(state.regions, by = c("State" = "abb")) %>%
...
```{r plotCOVIDGovernorParty, include = TRUE, fig.cap = "**Figure 10:** RDI of States Plotted by Age and Party of Governor"}
# By Governor Party
read_csv("./raw_data/states_governors.csv") %>%
  left_join(state.regions, by = c("State" = "abb")) %>%
...
```{r governorSexANOVA, include = TRUE, fig.cap = "**Table 2:** Analysis of Variance of Sex of State Governor vs RDI"}
...
```{r governorPartyANOVA, include = TRUE, fig.cap = "**Table 3:** Analysis of Variance of Sex of State Governor vs RDI"}
data <- read_csv("./raw_data/states_governors.csv") %>%
  left_join(state.regions, by = c("State" = "abb")) %>%
  select(region, Gov_Rep) %>%
  right_join(read_csv("./output/cdc_state_covid_age_race_data.csv"), by = "region") %>%
...
```{r ratePneumoniaVax, include = TRUE, fig.cap = "**Table 4:** Pneumonia Vaccination by Racial Group"}
...
```{r graphVax, include = TRUE, fig.cap = "**Figure: 11:** The correlation between pneumonia vaccination and per capita COVID deaths among the elderly non-White population"}
...
While statewide rates of pneumonia vaccination was significantly correlated with per capita COVID deaths, many other health variables (see figure 12; many others not shown) did not reach statistical significance. While it is possible that this reflects there actual lack of correlation, the more likely explanation is the lack in granularity in examining state-level data. Varying levels of [evidence](https://www.cdc.gov/coronavirus/2019-ncov/need-extra-precautions/evidence-table.html) exist for different conditions. Issues in reporting from different state- and county-level departments of health and coroners, and lack of availability of comorbidity data contribute to the difficulty in determining these correlations. Additionally, [lack of COVID testing and other factors](https://www.cdc.gov/nchs/nvss/vsrr/covid19/excess_deaths.htm) can lead to deaths not being correctly included in COVID counts.

```{r graphAsthma, include = TRUE, fig.cap = "**Figure 12:** The insignificant correlation between asthma and per capita COVID deaths among the elderly non-White population"}
data <- read_csv("./output/state_health.csv") %>%
  filter(age %in% c("65-74 years", "75-84 years") & !isWhite) %>%
  inner_join(state.regions, by = "region")
...
Using data taken from the BRFSS, the effect of various measures of population density were analyzed. Using the United States Office of Management and Budget definition of Metropolitan Statistical Areas (MSAs, figure 12), the correlations between living in metropolitan areas, including (figure 13) or excluding (figure 14) the suburbs, and per capita COVID deaths among the non-White population were determined.

![Metropolitan Statistical Areas](./figures/msa.gif)
**Figure 12:** Metropolitan Statistical Areas

\

```{r graphMetro, include = TRUE, fig.cap = "**Figure 13:** The correlations between living in MSAs and per capita COVID deaths among the non-White population"}
data <- read_csv("./output/state_health.csv") %>%
  filter(!isWhite) %>%
  inner_join(state.regions, by = "region")
...
```{r graphUrban, include = TRUE, fig.cap = "**Figure 14:** The correlations between living in MSAs (excluding suburbs) and per capita COVID deaths among the non-White population"}
data <- read_csv("./output/state_health.csv") %>%
  filter(!isWhite) %>%
  inner_join(state.regions, by = "region")
...
```{r graphRural, include = TRUE, fig.cap = "**Figure 15:** The correlations between living in rural counties and per capita COVID deaths among the non-White population"}
data <- read_csv("./output/state_health.csv") %>%
  filter(!isWhite) %>%
  inner_join(state.regions, by = "region")
...
```{r graphPopDensity, include = TRUE, fig.cap = "**Figure 16:** The correlations between population and per capita COVID deaths among the non-White population"}
...
As shown in figures 13-16, all measures of density were significantly correlated with the severity of COVID pandemic. Statewide measures of the percentage of the population living in metropolitan areas showed strong positive relationships with per capita deaths from COVID among non-White populations of all ages. The expected opposite trend held for the statewide percentage of the population living in rural areas (figure 15). More directly, population density (figure 16) showed the strongest correlation with per capita COVID deaths across all age groups, races, and variables investigated (data not shown). These data are in line with the expectation that human-to-human diseases kill most readily in areas where humans are closer together.
...
The following grab-bag of analyses are submitted for the reader's viewing pleasure, in honor of the dangers of careless data analysis and [spurious correlations](https://tylervigen.com/view_correlation?id=359).

\

```{r electionData, echo = FALSE, message = FALSE}
read_excel("./raw_data/states_votes.xlsx", sheet = "Table 2. Electoral &  Pop Vote") %>%
  select(1, 4, 5) %>%
  rename(State = 1, Trump = 2, Clinton = 3) %>%
  slice(4:54) %>%
  mutate(Trump = as.numeric(Trump), Clinton = as.numeric(Clinton)) %>%
  mutate(Margin = (Trump - Clinton) / (Trump + Clinton)) %>%
  full_join(read_csv("./raw_data/states_governors.csv"), by = "State") %>%
  select(State, Margin) %>%
  write_csv("./output/States_votes.csv")
```
...
tibbie <- read_sav("./raw_data/Dataset - Pew Research Center 2014 Religious Landscape Study National Telephone Survey - Version 1.1 - December 1 2016.sav") %>%
  ...
  mutate(Religion = if_else(Religion == 9 | Religion == 10 | Religion == 12, 0,
            if_else(Religion > 0 & Religion < 100, 1,
                  2))) %>%
  filter(!(Religion == 2)) %>%
  filter(God == 1 | God == 2) %>%
  mutate(God = if_else(God == 1, 1, 0)) %>%
  group_by(State)
...
state.regions %>%
  right_join(proportionNone, by = c("fips.numeric" = "State")) %>%
  right_join(proportionNoGod, by = c("fips.numeric" = "State")) %>%
  select(-c("abb", "fips.numeric", "fips.character")) %>%
	write_csv("./output/states_religion.csv")

rm(proportionNoGod, proportionNone)
```
...
monuments <- read_csv("./raw_data/Whose Heritage SF - Symbol Type Count by State.csv") %>%
  ...
  filter(State != "Grand Total")

georgia <- filter(monuments, State == "Georgia")

monuments <- mutate(monuments, Monuments = if_else(State == "GA", Monuments + georgia$Monuments, Monuments)) %>%
  filter(State != "Georgia") %>%
  left_join(state.regions, by = c("State" = "abb")) %>%
  ungroup() %>%
  select(region, Monuments)

full_join(monuments, statePop, by =  "region") %>%
  transmute(region, Per_Capita_Monuments = if_else(is.na(Monuments), 0, Monuments / pop)) %>%
	write_csv("./output/states_monuments.csv")

rm(georgia, monuments)
```

Words about monuments.

\

```{r wholeFoodsData, echo = FALSE, message = FALSE}
read_csv("./raw_data/states_whole_foods.csv") %>%
  inner_join(state.regions, by = c("State" = "abb")) %>%
  right_join(statePop, by = "region") %>%
  transmute(region, Per_Capita_Whole_Foods = Number_Whole_Foods / pop) %>%
	write_csv("./output/states_whole_foods.csv")
```

Words about Whole Foods.