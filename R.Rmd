...
computedStatesHealth <- states_health %>%
  ...
  summarize(Excercise = mean(EXERANY2)) %>% # 0-1; Exercise: Proportion of state that excercised at all in last month
  right_join(computedStatesHealth, by = "State")
...
proportionNoGod <- totalGodWeight %>%
  right_join(totalWeight) %>%
  transmute(State, Proportion_NoGod = (Total_Weight - Total_God_Weight) / Total_Weight) # These values appear to be "exactly" the same as the official cross tabs. How the same procedure can exist in a quantum state of correctly and incorrectly weighting is left as an excercise to the reader.
